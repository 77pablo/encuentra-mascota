...
- **3 funciones nuevas (jul-18), construidas en paralelo con 3 subagentes (una rama/worktree c/u) y fusionadas:**
  ...
  - ⚠️ **Pendiente prueba visual tuya (~2 min):** las pantallas de detalle (visto + reencuentro) NO se ejercitaron con datos reales porque la cuenta de prueba no tiene reportes y no quisimos escribir en prod. Inicio y "Mi zona de alerta" SÍ se verificaron en navegador. Para probar detalle: publicá un reporte de prueba → abrilo → "Lo vi por acá" y (en uno tuyo) "¡Volvió a casa!".
...
0. **Aplicar las 3 migraciones nuevas en Supabase** (SQL Editor, en orden): `0006_alertas_zona.sql` (tabla `alert_zones`), `0007_avistamientos.sql` (tabla `sightings`), `0008_final_feliz.sql` (columnas `reunida_en`/`final_feliz`/`final_foto` en `pets`). Hasta aplicar 0008, "Finales felices" y "¡Volvió a casa!" no funcionan (Inicio ya NO se rompe gracias al fix, pero la tira va vacía).
